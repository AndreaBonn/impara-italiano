/* ============================================================
   ui-pl.js — napisy interfejsu po polsku.

   Klucz ma postać obszar.element. Wartość to napis albo obiekt
   z formami liczby mnogiej — polski ma cztery kategorie i wybiera
   je Intl.PluralRules, nie warunek na n === 1.

   Terminy gramatyczne włoskie (passato prossimo, congiuntivo)
   i nazwy własne zostają bez zmian we wszystkich językach.
   ============================================================ */
LINGUAI.addUI("pl", {

  /* ---------------- Powłoka ---------------- */
  "app.title": "Impara l'Italiano 🇮🇹 — kurs włoskiego od A1 do C2",
  "app.description": "Kompletny kurs języka włoskiego po polsku: gramatyka, teoria, ćwiczenia, słuchanie i mówienie. Od A1 do poziomu native.",
  "app.skipToContent": "Przejdź do treści",
  "app.openMenu": "Otwórz menu",
  "app.mainNav": "Nawigacja główna",
  "app.tagline": "Włoski po polsku · A1 → C2",

  /* ---------------- Nawigacja ---------------- */
  "nav.path": "Ścieżka nauki",
  "nav.review": "Powtórki",
  "nav.talk": "Rozmowy na głos",
  "nav.grammar": "Gramatyka",
  "nav.verbs": "Odmiana czasowników",
  "nav.lexicon": "Mój słownik",
  "nav.progress": "Postępy",
  "nav.settings": "Ustawienia",

  /* ---------------- Motyw ---------------- */
  "theme.toggle": "Przełącz motyw",
  "theme.dark": "🌙 Tryb nocny",
  "theme.light": "☀️ Tryb dzienny",

  /* ---------------- Licznik w pasku ---------------- */
  "stats.days": { one: "dzień", few: "dni", many: "dni", other: "dni" },
  "stats.lessons": { one: "lekcja", few: "lekcje", many: "lekcji", other: "lekcji" },
  /* skrót, bo kafelek jest wąski — nie odmienia się przez liczbę */
  "stats.points": "pkt",

  /* ---------------- Wybór języka wyjaśnień ---------------- */
  "lang.group": "Język wyjaśnień",
  "lang.current": "Język wyjaśnień: {name}",
  "lang.partial": {
    one: "Nie udało się wczytać {n} pliku — część kursu została w poprzednim języku.",
    few: "Nie udało się wczytać {n} plików — część kursu została w poprzednim języku.",
    many: "Nie udało się wczytać {n} plików — część kursu została w poprzednim języku.",
    other: "Nie udało się wczytać {n} plików — część kursu została w poprzednim języku."
  },

  /* ---------------- Dostępność ---------------- */
  "a11y.listen": "Posłuchaj",
  "a11y.listenTo": "Posłuchaj {what}",
  "audio.noItalianVoice": "Brak włoskiego głosu w systemie: zdanie nie zostanie odczytane. Angielski lektor czytający po włosku nauczyłby złej wymowy. Nagrania kursu działają niezależnie od tego.",
  "audio.recordingFailed": "Nagranie jest w indeksie, ale plik się nie wczytał. Sprawdź, czy katalog audio/ jest obok index.html i czy strona jest serwowana z tego samego miejsca.",

  /* ---------------- Ścieżka nauki ---------------- */
  "path.noData": "Brak danych kursu",
  "path.noDataHint": "Nie udało się wczytać plików w katalogu <code>data/</code>.",
  "path.levelsGroup": "Poziomy CEFR",
  "path.loading": "Wczytuję materiał…",
  "path.loadError": "Nie udało się wczytać poziomu {code}",
  "path.loadErrorHint": "Sprawdź pliki: <code>{files}</code>.",
  "path.levelKicker": "Poziom {code} · {cefr}",
  "path.levelDone": "Poziom {code} ukończony 🌿",
  "path.levelDoneHint": "Przejdź do kolejnego poziomu albo wróć do powtórek, żeby utrwalić materiał.",
  "path.continue": "Kontynuuj",
  "path.start": "Zaczynamy →",
  "path.placementNudge": "Nie wiesz, od którego poziomu zacząć? Krótki test to podpowie.",

  /* ---------------- Lekcja ---------------- */
  "lesson.back": "← Ścieżka",
  "lesson.notFound": "Nie znaleziono lekcji",
  "lesson.notFoundHint": "Wróć do ścieżki nauki.",
  "lesson.objectives": "Po tej lekcji będziesz umieć:",
  "lesson.theory": "Teoria",
  "lesson.trapLabel": "Uwaga, pułapka:",
  "lesson.contrastLabel": "Dla Polaków:",
  "lesson.tipLabel": "Wskazówka:",
  "lesson.grammar": "Gramatyka",
  "lesson.vocab": "Słownictwo",
  "lesson.playAll": "🔊 Odsłuchaj całą listę",
  "lesson.saveAll": "⭐ Dodaj wszystko do powtórek",
  "lesson.addToReview": "Dodaj do powtórek",
  "lesson.savedVocab": {
    one: "Dodano {n} słówko do powtórek.",
    few: "Dodano {n} słówka do powtórek.",
    many: "Dodano {n} słówek do powtórek.",
    other: "Dodano {n} słówek do powtórek."
  },
  "lesson.dialogue": "Dialog: {title}",
  "lesson.playDialogue": "▶️ Odtwórz cały dialog",
  "lesson.culture": "Okiem Włocha",
  "lesson.exercises": "Ćwiczenia",
  "lesson.end.perfect": "Bezbłędnie. Perfetto!",
  "lesson.end.great": "Bardzo dobrze — materiał opanowany.",
  "lesson.end.pass": "Zaliczone. Kilka rzeczy warto powtórzyć.",
  "lesson.end.fail": "Jeszcze nie zaliczone (potrzeba 70%). Wróć do teorii i spróbuj ponownie.",
  "lesson.again": "Powtórz lekcję",
  "lesson.next": "Dalej: {title} →",

  /* ---------------- Ćwiczenia: nazwy typów ---------------- */
  "ex.type.mcq": "wybór",
  "ex.type.multi": "wybór wielokrotny",
  "ex.type.fill": "uzupełnij lukę",
  "ex.type.cloze": "uzupełnij tekst",
  "ex.type.trans": "tłumaczenie",
  "ex.type.order": "ułóż zdanie",
  "ex.type.match": "połącz w pary",
  "ex.type.conj": "odmiana",
  "ex.type.gender": "rodzajnik",
  "ex.type.listen": "ze słuchu",
  "ex.type.speak": "wymowa",
  "ex.type.dialogue": "rozmowa",
  "ex.type.truefalse": "prawda / fałsz",

  /* ---------------- Ćwiczenia: wspólne ---------------- */
  "ex.num": "Ćwiczenie {n} · {type}",
  "ex.check": "Sprawdź",
  "ex.done.ok": "Dobrze ✓",
  "ex.done.checked": "Sprawdzone",
  "ex.bravo": "Brawo! 🎉",
  "ex.correctIs": "Poprawnie: „{answer}”",
  "ex.notYet": "Jeszcze nie.",
  "ex.answersGroup": "Odpowiedzi",
  "ex.pickOne": "Wybierz odpowiedź.",
  "ex.pickAtLeastOne": "Zaznacz przynajmniej jedną odpowiedź.",
  "ex.multiHint": "Zaznacz wszystkie poprawne odpowiedzi.",
  "ex.hintLabel": "Podpowiedź: {hint}",
  "ex.almost": "Prawie! Sprawdź pisownię (literówka lub akcent).",
  "ex.ph.toIt": "Napisz po włosku",
  "ex.ph.toBase": "Napisz po polsku",
  "ex.ph.answer": "Wpisz odpowiedź",
  "ex.unknownType": "Nieznany typ ćwiczenia: {t}",

  /* ---------------- Ćwiczenia: poszczególne typy ---------------- */
  "ex.cloze.prompt": "Uzupełnij luki.",
  "ex.order.prompt": "Ułóż zdanie po włosku:",
  "ex.order.yourSentence": "Twoje zdanie",
  "ex.order.clear": "Wyczyść",
  "ex.order.empty": "Ułóż zdanie z klocków.",
  "ex.match.prompt": "Połącz włoskie wyrażenia z polskimi odpowiednikami.",
  "ex.match.mistakes": "Liczba pomyłek: {n}.",
  "ex.conj.prompt": "Odmień: {verb} — {tense}",
  "ex.gender.prompt": "Dopasuj właściwy rodzajnik.",
  "ex.listen.prompt": "Posłuchaj i zapisz, co słyszysz.",
  "ex.listen.play": "🔊 Odtwórz",
  "ex.listen.slow": "🐢 Wolniej",
  "ex.listen.ph": "Zapisz po włosku",
  "ex.speak.prompt": "Powiedz na głos po włosku:",
  "ex.speak.mic": "Nagraj wypowiedź",
  "ex.speak.hint": "Kliknij mikrofon i przeczytaj zdanie.",
  "ex.speak.noStt": "Twoja przeglądarka nie obsługuje rozpoznawania mowy. Posłuchaj wzoru, powtórz na głos, a potem przepisz zdanie z pamięci.",
  "ex.speak.ph": "Przepisz zdanie",
  "ex.speak.pass": "Zalicz",
  "ex.speak.recordFirst": "Najpierw nagraj wypowiedź.",
  "ex.speak.ok": "Wymowa rozpoznana poprawnie.",
  "ex.speak.retry": "Rozpoznanie mowy bywa surowe dla obcego akcentu — posłuchaj wzoru i spróbuj jeszcze raz w spokojnym tempie.",
  "ex.dialogue.prompt": "Posłuchaj i odpowiedz.",
  "ex.dialogue.yourTurn": "Twoja kolej — {task}",
  "ex.dialogue.pickAnswer": "wybierz odpowiedź",
  "ex.dialogue.mistakes": "Pomyłki: {n}. Przeczytaj dialog jeszcze raz na głos.",
  "ex.dialogue.allOk": "Cała rozmowa poprawna.",
  "ex.truefalse.true": "Prawda (vero)",
  "ex.truefalse.false": "Fałsz (falso)",

  /* ---------------- Rozpoznawanie mowy ---------------- */
  "ex.stt.listening": "Słucham…",
  "ex.stt.denied": "Brak zgody na mikrofon. Zezwól w ustawieniach przeglądarki.",
  "ex.stt.noConsent": "Bez zgody rozpoznawanie mowy nie działa. Odpowiedz pisemnie w polu poniżej.",
  "ex.stt.failed": "Nie udało się nagrać. Spróbuj ponownie.",
  "ex.stt.nothing": "Nic nie usłyszałam. Spróbuj jeszcze raz.",
  "ex.stt.heard": "Usłyszałam: {text}",

  /* ---------------- Powtórki (SRS) ---------------- */
  "srs.nothingDue": "Nic do powtórzenia 🌿",
  "srs.allResting": "Wszystkie {n} fiszek jest w spoczynku. Wróć jutro albo dodaj nowe słówka gwiazdką w lekcji.",
  "srs.deckEmpty": "Twoja talia jest pusta. Oznacz słówka gwiazdką ⭐ w dowolnej lekcji, a wrócą tu we właściwym momencie.",
  "srs.howTitle": "System powtórek",
  "srs.howText": "Fiszki wracają w rosnących odstępach, od jednego dnia do wielu miesięcy, zależnie od tego, jak pewnie odpowiadasz. To algorytm FSRS, ten sam, który liczy terminy w Anki.",
  "srs.dueToday": "Do powtórzenia dziś: {n}",
  "srs.gradeHonestly": "Odpowiadaj na głos albo w myślach, potem oceń szczerze — od tego zależy, kiedy fiszka wróci.",
  /* ---------------- Szukanie w kursie ---------------- */
  "search.open": "Szukaj w kursie",
  "search.kicker": "SZUKANIE",
  "search.title": "Gdzie to było?",
  "search.intro": "Przeszukuje tytuły lekcji, słownictwo, hasła gramatyczne i rozmowy. Po włosku albo po polsku, z akcentami lub bez.",
  "search.label": "Szukana fraza",
  "search.ph": "np. magari, congiuntivo, dworzec",
  "search.go": "Szukaj",
  "search.count": { one: "{n} trafienie", few: "{n} trafienia", many: "{n} trafień", other: "{n} trafień" },
  "search.partial": "Nie udało się wczytać poziomów: {levels}. Wyniki mogą być niepełne.",
  "search.tooShortTitle": "Wpisz co najmniej dwa znaki",
  "search.tooShortText": "Przy jednym znaku trafień byłoby zbyt wiele, żeby były do czegoś.",
  "search.noneTitle": "Nic takiego w kursie nie ma",
  "search.noneText": "Sprawdź pisownię albo poszukaj krótszego fragmentu słowa.",
  "search.kind.lesson": "lekcja",
  "search.kind.vocab": "słówko",
  "search.kind.grammar": "gramatyka",
  "search.kind.talk": "rozmowa",
  "keys.barLabel": "Znaki włoskie",

  /* ---------------- Test poziomujący ---------------- */
  /* ---------------- Przewodnik ---------------- */
  "nav.guide": "Przewodnik",
  "guide.kicker": "PRZEWODNIK",
  "guide.title": "Jak korzystać z kursu",
  "guide.intro": "Piętnaście zakładek i żadna nie jest obowiązkowa. Ta strona mówi, w jakiej kolejności to wszystko ma sens i co robi każdy ekran.",
  "guide.tocLabel": "Spis treści",
  "guide.more": "Jak to działa →",
  "guide.open": "Otwórz",

  "guide.inizio.h": "Od czego zacząć",
  "guide.inizio.p1": "Jeśli nigdy nie uczyłeś się włoskiego, idź prosto do pierwszej lekcji A1. Kurs jest ułożony tak, żeby dało się zacząć od zera, i nie trzeba wcześniej niczego ustawiać.",
  "guide.inizio.p2": "Jeśli coś już umiesz, zrób test poziomujący. Skacze między poziomami zamiast iść po kolei, więc kończy się szybko, a na koniec odhacza lekcje z niższych poziomów. Wynik jest propozycją: możesz go odrzucić i zacząć od początku.",

  "guide.ordine.h": "Kolejność, która ma sens",
  "guide.ordine.p1": "Poziomy idą od A1 do C2 i gramatyka jest kumulatywna: A2 zakłada opanowane A1, B1 zakłada A2. Nic nie blokuje przeskoku, ale od tego miejsca kolejne lekcje przestają tłumaczyć się same.",
  "guide.ordine.p2": "Wewnątrz poziomu jednostki idą po kolei, tak samo lekcje w jednostce. Test na końcu jednostki sprawdza ją całą, nie ostatnią lekcję.",

  "guide.lezione.h": "Jak wygląda lekcja",
  "guide.lezione.p1": "Każda lekcja ma tę samą budowę: teoria, gramatyka z tabelą i przykładami, słownictwo, dialog, notatka kulturowa, ćwiczenia. Teorię czyta się raz i zostaje na stronie lekcji, więc można do niej wrócić w trakcie ćwiczeń.",
  "guide.lezione.p2": "Lekcja zalicza się od 70 procent poprawnych odpowiedzi. Poniżej progu zostaje otwarta i można ją powtórzyć — nic nie przepada, a wynik liczy się od nowa.",
  "guide.lezione.p3": "Każda zła odpowiedź zakłada kartę w quaderno błędów i wraca do ciebie w Powtórkach. Nie musisz nic notować ani niczego zaznaczać.",

  "guide.oggi.h": "Dziś: dziesięć minut bez wybierania",
  "guide.oggi.p1": "Zakładka Dziś układa sesję sama: najpierw twoje pomyłki, potem trening tematu, który idzie ci najsłabiej, na koniec fiszki do powtórzenia. To jest droga na dzień, w którym nie chcesz decydować, co robić.",
  "guide.oggi.p2": "Sesja ma zawsze czym się wypełnić, także wtedy, gdy nie masz zaległości: zadania z generatora nie kończą się nigdy.",

  "guide.ripasso.h": "Powtórki: dwie talie, dwa rytmy",
  "guide.ripasso.p1": "Fiszki ze słownictwem chodzą algorytmem FSRS: słowo, które znasz, wraca coraz rzadziej, słowo, na którym się potykasz, wraca nazajutrz. Odstęp liczy się z twoich odpowiedzi, nie z kalendarza.",
  "guide.ripasso.p2": "Quaderno błędów to druga talia. Trafia do niej dokładnie to ćwiczenie, które pomyliłeś, podpisane nazwą zagadnienia — widać więc nie tylko co poszło źle, ale i czego dotyczyło.",
  "guide.ripasso.p3": "Licznik przy zakładce Powtórki pokazuje obie talie razem. Zero znaczy „na dziś nie ma zaległości”, a nie „koniec nauki”.",

  "guide.esame.h": "Egzamin",
  "guide.esame.p1": "Zakładka Egzamin to symulacja CILS: sekcje idą w ustalonej kolejności, każda ma swój limit czasu, odpowiedzi wpisuje się w siatkę zamiast sprawdzać po jednej. Całe polecenia są po włosku, bo na prawdziwym egzaminie też będą.",
  "guide.esame.p2": "Nie ma sensu podchodzić wcześnie: to jest sprawdzian poziomu, który już masz, a nie sposób, żeby się go nauczyć. Rozsądny moment to koniec poziomu, do którego zdajesz.",

  "guide.mappa.h": "Wszystkie ekrany po kolei",
  "guide.mappa.p1": "Piętnaście pozycji z paska plus cztery ekrany, których w pasku nie ma, bo wchodzi się do nich z Treningu i z wyszukiwarki. Krótko, co robi każdy.",

  "guide.backup.h": "Kopia zapasowa: nie ma konta, jest plik",
  "guide.backup.p1": "Kurs nie ma konta ani synchronizacji. Wszystko, czego się nauczyłeś, leży w tej przeglądarce, na tym urządzeniu, i wyczyszczenie danych strony kasuje to bezpowrotnie.",
  "guide.backup.p2": "Co dziesięć ukończonych lekcji kurs sam przypomni o pobraniu kopii. Plik zapisujesz, gdzie chcesz, a wczytujesz go w Ustawieniach — to również jedyny sposób, żeby przenieść naukę na inne urządzenie.",

  "guide.bloccato.h": "Kiedy coś nie wchodzi",
  "guide.bloccato.p1": "Nie powtarzaj tej samej lekcji trzeci raz z rzędu. Zrób Trening tego zagadnienia — zadania powstają tam z reguł, więc za każdym razem są inne — a do lekcji wróć nazajutrz.",
  "guide.bloccato.p2": "Jeśli nie wchodzi wymowa, otwórz Pary minimalne albo Powtarzanie: tam ćwiczy się ucho i usta, nie regułę. A jeśli nie pamiętasz, gdzie coś było, wyszukiwarka nad paskiem przeszukuje cały kurs.",

  "guide.s.oggi": "Gotowa sesja na dziś: pomyłki, trening, fiszki.",
  "guide.s.percorso": "Poziomy, jednostki i lekcje. Stąd prowadzi cała nauka.",
  "guide.s.ripasso": "Fiszki i quaderno błędów, czyli wszystko, co wraca.",
  "guide.s.allenamento": "Zadania z reguł, nie z listy, więc nie kończą się po czterech.",
  "guide.s.conversazione": "Kurs mówi po włosku, ty odpowiadasz do mikrofonu.",
  "guide.s.chiacchiere": "Rozmowa bez scenariusza z modelem: mówisz swoje, rozmówca odpowiada po włosku i osobno poprawia. Wymaga klucza i osobnej zgody; nic tu nie jest punktowane.",
  "guide.s.grammatica": "Pełny sylabus gramatyczny A1 → C2, do czytania.",
  "guide.s.coniugatore": "Odmiana dowolnego czasownika, także zwrotnego.",
  "guide.s.lessico": "Słowa oznaczone gwiazdką, z terminem następnej powtórki.",
  "guide.s.shadowing": "Posłuchaj, powtórz, odsłuchaj oba nagrania po sobie.",
  "guide.s.velocita": "To samo zdanie w normalnym tempie. Spowolnienie jest kołem ratunkowym.",
  "guide.s.esame": "Symulacja CILS z limitem czasu i historią podejść.",
  "guide.s.falsi": "Słowa, które znaczą co innego, niż podpowiada twój język.",
  "guide.s.copertura": "Ile najczęstszych włoskich form już rozumiesz.",
  "guide.s.progressi": "Passa, punkty i wykres dni. Wszystko liczone lokalnie.",
  "guide.s.impostazioni": "Głos, tempo, akcenty, kopia zapasowa, test poziomujący.",
  "guide.s.suoni": "Dwa wyrazy, jeden dźwięk różnicy. Najpierw ucho, potem usta.",
  "guide.s.lettura": "Dłuższe teksty: trzymanie sensu przez kilka zdań z rzędu.",
  "guide.s.scrittura": "Wypracowania sprawdzane z tego, co da się sprawdzić: użytych konstrukcji.",
  "guide.s.cerca": "Szuka po lekcjach, słownictwie, gramatyce i rozmowach.",
  "guide.s.privacy": "Co kurs zapisuje, co wychodzi poza twoje urządzenie i jak to cofnąć.",

  /* ---------------- Ekran powitalny ---------------- */
  "welcome.kicker": "POCZĄTEK",
  "welcome.title": "Witaj w kursie włoskiego",
  "welcome.intro": "Zanim zaczniemy: od czego chcesz wyjść? Wyboru nie robisz na zawsze, poziom da się zmienić w każdej chwili.",
  "welcome.zeroTitle": "Zaczynam od zera",
  "welcome.zeroHint": "Pierwsza lekcja A1, od przywitania i zamówienia kawy.",
  "welcome.zeroGo": "Zaczynamy →",
  "welcome.testTitle": "Coś już umiem",
  "welcome.testHint": "Krótki test wskaże poziom i odhaczy to, co masz za sobą.",
  "welcome.lookTitle": "Najpierw się rozejrzę",
  "welcome.lookHint": "Otwiera ścieżkę nauki. Wybór poziomu może zaczekać.",
  "welcome.lookGo": "Pokaż kurs",
  "welcome.guide": "Jak to działa? Przewodnik po kursie",

  "place.kicker": "POZIOM",
  "place.title": "Od czego zacząć",
  "place.intro": "Krótki test, żeby nie przeklikiwać stu lekcji, których już nie potrzebujesz.",
  "place.how": "Około {n} zadań w trzech rundach. Test skacze po poziomach zamiast iść od A1, więc kończy się szybko także wtedy, gdy umiesz sporo.",
  "place.previous": "Poprzedni wynik: {level}. Zaliczono wtedy {n} lekcji.",
  "place.start": "Zacznij test",
  "place.running": "Odpowiadaj spokojnie. Jeśli nie wiesz, zgaduj — to też jest informacja.",
  "place.loading": "Wczytuję zadania ze wszystkich poziomów…",
  "place.loadFailed": "Nie udało się wczytać poziomów: {levels}. Wynik może być zaniżony przez problem z siecią, nie przez twoją wiedzę.",
  "place.progress": "Zadanie {i} · poziom {level}",
  "place.next": "Dalej",
  "place.noItems": "Poziom {level} nie ma zadań nadających się do testu.",
  "place.result": "Twój poziom to {level}. Trafionych {hit} z {asked}.",
  "place.willMark": "Przyjęcie wyniku oznaczy jako zaliczone {n} lekcji z niższych poziomów. Poziom {level} zostaje do zrobienia.",
  "place.accept": "Zacznij od {level}",
  "place.fromScratch": "Wolę od początku",
  "place.applied": "Oznaczono {n} lekcji. Punktów za nie nie ma — nie zrobiłeś ich.",

  /* ---------------- Pisanie ---------------- */
  "write.kicker": "PISANIE",
  "write.title": "Pisanie po włosku",
  "write.hubHint": "Wypracowania z mierzalnym wymaganiem i tłumaczenia zdanie po zdaniu.",
  "write.intro": "Nikt tu nie oceni twojego tekstu i kurs tego nie udaje. Sprawdzane jest to, co da się sprawdzić: czy użyłeś konstrukcji, o które prosiło zadanie. Resztę porównasz z modelem.",
  "write.kindCompose": "wypracowanie",
  "write.kindTranslate": "tłumaczenie",
  "write.open": "Otwórz",
  "write.reopen": "Wróć do tekstu",
  "write.saved": "zapisane, {n} słów",
  "write.back": "← Zadania",
  "write.yourText": "Twój tekst",
  "write.placeholder": "Napisz po włosku, co najmniej {n} słów…",
  "write.words": "Słów: {n}",
  "write.mustUse": "Zadanie prosi o użycie",
  "write.reqVerb": "{verb} w czasie: {tense}",
  "write.reqWord": "słowo „{word}”",
  "write.reqAny": "jedno z: {list}",
  "write.foundAs": "znalezione jako: {form}",
  "write.yes": "jest",
  "write.no": "brak",
  "write.check": "Sprawdź, czego użyłem",
  "write.whatWeChecked": "Co dało się sprawdzić",
  "write.summary": "Spełnione {found} z {total} wymagań. Napisałeś {words} słów, oczekiwane minimum to {min}.",
  "write.notChecked": "Czego ten kurs NIE sprawdził",
  "write.notCheckedText": "Sensu, spójności i brzmienia nie sprawdza tu żaden automat i nie udaje, że sprawdza. Poniższe pytania zadaj sobie sam, najlepiej po przeczytaniu tekstu na głos.",
  "write.showModel": "Pokaż tekst modelowy",
  "write.model": "Tekst modelowy",
  "write.modelNote": "To jedna z możliwych wersji, nie wzorzec do przepisania. Porównaj, co on robi inaczej niż ty, i dlaczego.",
  "write.askOpinion": "Poproś model o opinię",
  "write.askingOpinion": "Czytam twój tekst…",
  "write.opinion": "Opinia modelu",
  "write.opinionNote": "To zdanie modelu, nie ocena kursu. Nie wpływa na twoje postępy ani na fiszki, i potrafi się mylić — czytaj je jak uwagę kolegi, nie jak poprawkę nauczyciela.",
  "write.opinionFailed": "Nie udało się dostać opinii. Sprawdź klucze w Ustawieniach albo spróbuj później.",
  "write.transDone": "Koniec tłumaczenia. Pomyłki wrócą w zakładce Błędy.",

  /* ---------------- Czytanie i słuchanie ciągłe ---------------- */
  "read.kicker": "TEKSTY",
  "read.title": "Czytanie i słuchanie",
  "read.hubHint": "Dłuższe teksty z pytaniami i dyktandem. Zdanie po zdaniu da się dojść do B1 i tam stanąć.",
  "read.intro": "Prawdziwy włoski nie przychodzi po jednym zdaniu. Te teksty ćwiczą to, czego pojedyncze ćwiczenia nie ćwiczą: trzymanie sensu przez kilka zdań z rzędu.",
  "read.open": "Otwórz",
  "read.long": "{n} zdań",
  "read.resume": "Wznów od zdania {n}",
  "read.back": "← Teksty",
  "read.backList": "Inne teksty",
  "read.modesLabel": "Tryb pracy z tekstem",
  "read.modeRead": "Czytaj",
  "read.modeListen": "Słuchaj",
  "read.modeDictation": "Dyktando",
  "read.playAll": "▶️ Odtwórz cały tekst",
  "read.glossary": "Trudniejsze słowa",
  "read.listenHint": "Tekst jest schowany. Posłuchaj całości, potem przejdź do pytań — „rozumiem czytając” to nie to samo, co „rozumiem”.",
  "read.toQuestions": "Przejdź do pytań",
  "read.questions": "Pytania do tekstu",
  "read.score": "Trafionych {hit} z {n}.",
  "read.dictProgress": "Zdanie {i} z {n}",
  "read.next": "Dalej",
  "read.dictDone": "Dyktando zakończone.",
  "read.again": "Jeszcze raz",

  /* ---------------- Rozróżnianie dźwięków ---------------- */
  "sound.kicker": "SŁUCH",
  "sound.title": "Pary minimalne",
  "sound.open": "Ćwicz słuch",
  "sound.hubHint": "Dwa wyrazy różniące się jednym dźwiękiem. Słyszysz jeden, wskazujesz który.",
  "sound.intro": "Zanim zaczniesz poprawiać wymowę, sprawdź, czy w ogóle słyszysz różnicę. Dopóki ucho jej nie łapie, usta jej nie zrobią.",
  "sound.setIntro": "Posłuchaj i wskaż, który wyraz padł.",
  "sound.pairs": { one: "{n} para", few: "{n} pary", many: "{n} par", other: "{n} par" },
  "sound.start": "Słuchaj",
  "sound.progress": "Para {i} z {n}",
  "sound.next": "Dalej",
  "sound.done": "Koniec podejścia. Wróć do tego zbioru za kilka dni.",
  "sound.doneGood": "Ucho już to łapie. Teraz spróbuj sam wymówić obie formy.",
  "sound.again": "Jeszcze raz",
  "sound.backList": "Inne dźwięki",
  "ex.type.minpair": "para minimalna",
  "ex.minpair.prompt": "Który wyraz słyszysz?",
  "ex.minpair.play": "🔊 Odtwórz",
  "ex.minpair.listenFirst": "Najpierw posłuchaj — bez tego to rzut monetą.",

  /* ---------------- Sesja dnia ---------------- */
  "nav.today": "Dziś",
  "today.kicker": "DZIŚ",
  "today.title": "Dziesięć minut, bez wybierania",
  "today.intro": "Sesja układa się sama z tego, co akurat wymaga uwagi: najpierw pomyłki, potem trening słabego punktu, na koniec fiszki.",
  "today.alreadyDone": "Dzisiejszą sesję masz już za sobą. Możesz zrobić kolejną, nic się nie zepsuje.",
  "today.partErrors": "Zadania, w których się pomyliłeś",
  "today.partDrills": "Trening: {topic}",
  "today.partCards": "Fiszki do powtórzenia",
  "today.start": "Zacznij ({n})",
  "today.progress": "{i} z {n}",
  "today.stepError": "twoja pomyłka",
  "today.stepDrill": "trening",
  "today.stepCards": "Fiszki",
  "today.next": "Dalej",
  "today.done": "Sesja dnia zamknięta.",
  "today.nextLesson": "Dalej: {lesson}",

  /* ---------------- Trening: ćwiczenia z generatora ---------------- */
  "nav.train": "Trening",
  "train.kicker": "TRENING",
  "train.title": "Tyle powtórzeń, ile potrzebujesz",
  "train.intro": "Zadania powstają tu z reguł, nie z listy, więc nie kończą się po czterech. Wybierz, co chcesz przećwiczyć.",
  "train.start": "Ćwicz",
  "train.endless": "Każde podejście losuje nowe zadania. Pomyłki trafiają do zakładki Błędy w Powtórkach.",
  "train.topic.prep-art": "Przyimek z rodzajnikiem",
  "train.topic.accordo": "Zgodność przymiotnika",
  "train.topic.ausiliare": "Essere czy avere",
  "train.topic.pronomi": "Zaimki dopełnienia",
  "train.topic.numeri": "Liczebniki",
  "train.topic.date": "Daty",
  "train.topic.ore": "Godziny",
  "train.progress": "Zadanie {i} z {n}",
  "train.next": "Dalej",
  "train.done": "Podejście zakończone.",
  "train.again": "Jeszcze raz",
  "train.backHub": "Wybierz inne",

  /* Polecenia generatorów. Włoskie słowa wchodzą jako zmienne, więc
     tłumaczy się tylko ramkę zdania, nigdy sam materiał językowy. */
  "drill.prepArt.q": "Połącz w jedno: <b>{prep}</b> + rodzajnik od <b>{noun}</b>",
  "drill.accordo.q": "Dopasuj przymiotnik: <b>{group}</b> + <b>{adj}</b>",
  "drill.pron.q": "Zastąp dopełnienie zaimkiem: <b>{sentence}</b>",
  "drill.aux.q": "Jakiego czasownika posiłkowego wymaga <b>{verb}</b> w passato prossimo?",
  "drill.aux.both": "<b>{verb}</b> przyjmuje oba — zaznacz oba.",
  "drill.aux.whyAvere": "Czasownik przechodni: bierze avere.",
  "drill.aux.whyEssere": "Czasownik ruchu albo zmiany stanu: bierze essere.",
  "drill.aux.whyRefl": "Zwrotny, więc zawsze essere — także wtedy, gdy bez „si” brałby avere.",
  "drill.aux.whyBoth": "Z avere jest przechodni („ha cambiato casa”), z essere nieprzechodni („è cambiato”). Inne posiłkowe, inne znaczenie.",
  "drill.num.q": "Napisz słownie: <b>{n}</b>",
  "drill.date.q": "Napisz datę po włosku: <b>{day} {month}</b>",
  "drill.ore.q": "Która godzina? <b>{time}</b>",

  /* ---------------- Powtórki: zakładki ---------------- */
  "review.title": "Wracamy do tego, co się chwieje",
  "review.intro": "Dwie talie. Fiszki to słówka, które sam oznaczyłeś. Błędy zbierają się same, z zadań, w których się potknąłeś.",
  "review.tabsLabel": "Rodzaj powtórki",
  "review.tabCards": "Fiszki",
  "review.tabErrors": "Błędy",

  /* ---------------- Quaderno błędów ---------------- */
  "err.emptyTitle": "Nie ma jeszcze czego powtarzać",
  "err.emptyText": "Za każdym razem, gdy pomylisz się w ćwiczeniu, zadanie wraca tutaj — dokładnie to samo, nie podobne. Wystarczą dwie poprawne odpowiedzi z rzędu, żeby wyszło z obiegu.",
  "err.byTag": "Na czym się potykasz",
  "err.cardsIn": { one: "{n} zadanie", few: "{n} zadania", many: "{n} zadań", other: "{n} zadań" },
  "err.dueNow": "na teraz: {n}",
  "err.resting": "w spoczynku",
  "err.dueTitle": {
    one: "Jedno zadanie czeka na powtórkę",
    few: "{n} zadania czekają na powtórkę",
    many: "{n} zadań czeka na powtórkę",
    other: "{n} zadań czeka na powtórkę"
  },
  "err.dueText": "To te same ćwiczenia, w których się pomyliłeś, z tej samej lekcji.",
  "err.start": "Zacznij powtórkę",
  "err.restingTitle": {
    one: "Jedno zadanie w spoczynku",
    few: "{n} zadania w spoczynku",
    many: "{n} zadań w spoczynku",
    other: "{n} zadań w spoczynku"
  },
  "err.restingText": "Wrócą, gdy przyjdzie ich termin. Odstępy rosną tak samo jak przy fiszkach.",
  "err.progress": "Zadanie {i} z {n}",
  "err.fromLesson": "z lekcji: {lesson}",
  "err.next": "Dalej",
  "err.done": "Powtórka błędów zakończona.",
  "err.gone": {
    one: "Jedno zadanie zmieniło się w kursie i jego karta została usunięta.",
    few: "{n} zadania zmieniły się w kursie i ich karty zostały usunięte.",
    many: "{n} zadań zmieniło się w kursie i ich karty zostały usunięte.",
    other: "{n} zadań zmieniło się w kursie i ich karty zostały usunięte."
  },
  "err.again": "Jeszcze raz",

  "srs.sessionDone": "Sesja powtórek zakończona.",
  "srs.nextBatch": "Kolejna partia",
  "srs.cardOf": "Fiszka {i} z {n}",
  "srs.howInItalian": "Jak to powiesz po włosku?",
  "srs.ph": "Po włosku",
  "srs.right": "Dobrze!",
  "srs.wrong": "Poprawnie:",
  "srs.grade0": "Nie pamiętam",
  "srs.grade3": "Z trudem",
  "srs.grade4": "Dobrze",
  "srs.grade5": "Łatwo",

  /* ---------------- Rozmowy na głos ---------------- */
  "talk.kicker": "Mówienie",
  "talk.intro": "Aplikacja mówi po włosku, Ty odpowiadasz do mikrofonu. Scenariusze z prawdziwego życia — bar, sklep, dworzec, rozmowa o pracę.",
  "talk.noSttLabel": "Uwaga:",
  "talk.noStt": "Twoja przeglądarka nie obsługuje rozpoznawania mowy. Rozmowy zadziałają w trybie pisanym. Pełne mówienie działa w Chrome, Edge i Safari 16+.",
  "talk.notFound": "Nie znaleziono rozmowy",
  "talk.backToList": "← Wszystkie rozmowy",
  "talk.yourTurn": "Twoja kolej: {task}",
  "talk.speak": "Mów",
  "talk.tapAndSpeak": "Kliknij i powiedz po włosku.",
  "talk.orType": "…albo wpisz odpowiedź",
  "talk.typeAnswer": "Wpisz odpowiedź",
  "talk.send": "Wyślij",
  "talk.reveal": "Pokaż odpowiedź",
  "talk.tryAgain": "To jeszcze nie to. Spróbuj jeszcze raz.",
  "talk.emptyAnswer": "Napisz albo powiedz odpowiedź.",
  "talk.defaultClosing": "Rozmowa zakończona. Powtórz ją za kilka dni — płynność bierze się z powtarzania.",
  "talk.again": "Jeszcze raz",
  "talk.others": "Inne rozmowy",
  "talk.chooseOne": "Tutaj rozmowa się rozgałęzia. Wybierz, co powiesz, albo powiedz to własnymi słowami.",
  "talk.otherBranch": "Wróć do wyboru",
  "nav.exam": "Egzamin",
  "cils.none": "Nie ma jeszcze symulacji egzaminu.",
  "cils.kicker": "CILS B1 Cittadinanza",
  "cils.title": "Symulacja egzaminu",
  "cils.intro": "Cały egzamin jest po włosku, łącznie z poleceniami. Tutaj też: inaczej ćwiczyłbyś inne zadanie niż to, które cię czeka.",
  "cils.limitLabel": "Zanim zaczniesz:",
  "cils.limit": "Ten symulator liczy punkty z dwóch sprawności na cztery, ze słuchania i z czytania. Wypracowanie wraca jako lista elementów zadania, które są i których brakuje, a wypowiedź ustna nie wchodzi do wyniku w ogóle: te dwie siatki ocenia człowiek, a automat udawałby ocenę, której nie ma.",
  "cils.structure": "Cztery sprawności, sześć prób: dwie ze słuchania (30 minut), dwie z czytania z gramatyką (40 minut), wypracowanie na 80-120 słów (40 minut) i rozmowa (około 10 minut).",
  "cils.thresholdSource": "Próg 7 punktów na 12 w każdej sprawności nie jest publikowany przez Centro CILS dla tego modułu: pochodzi z ich własnego dokumentu proceduralnego i z materiału państwowej siedziby egzaminacyjnej. Dwa źródła instytucjonalne podają zamiast tego skalę egzaminu standardowego, sprzeczną z maksimum tego modułu.",
  "cils.sectionsLine": "Słuchanie, czytanie, pisanie, mówienie",
  "cils.start": "Zacznij",
  "cils.stepOf": "Sekcja {i} z {n}",
  "cils.minutes": "{n} minut",
  "cils.sec.ascolto": "Słuchanie",
  "cils.sec.lettura": "Czytanie i gramatyka",
  "cils.sec.scritta": "Wypowiedź pisemna",
  "cils.sec.orale": "Wypowiedź ustna",
  "cils.testN": "Próba {n}",
  "cils.playText": "▶ Tekst {n}",
  "cils.twice": "Każdego tekstu możesz posłuchać dwa razy, tak jak na egzaminie.",
  "cils.playedTwice": "Wysłuchane dwa razy",
  "cils.expired": "Czas tej sekcji minął. Policzone są odpowiedzi, które zdążyłeś zaznaczyć.",
  "cils.closeSection": "Zamknij sekcję",
  "cils.timeWarn": "Zostało {m} minut.",
  "cils.timeUp": "Czas minął.",
  "cils.wordsOf": "{n} słów (wymagane od {min} do {max})",
  "cils.outOfRange": "Poza limitem.",
  "cils.oralLabel": "Bez oceny.",
  "cils.oralNotScored": "Tej części symulator nie ocenia. Nagraj się i mów przez dwie, trzy minuty; po egzaminie odsłuchasz siebie i spiszesz, co powiedziałeś.",
  "cils.oralIntro": "Krótka prezentacja",
  "cils.oralTopic": "Wybierz temat rozmowy",
  "cils.record": "🎤 Nagraj",
  "cils.stopRec": "■ Zatrzymaj",
  "cils.recordAgain": "🎤 Nagraj jeszcze raz",
  "cils.recording": "Nagrywam…",
  "cils.recorded": "Nagrane, {s} sekundy.",
  "cils.oralNoText": "Nie spisałeś, co powiedziałeś, więc tej części nie ma czego sprawdzić.",
  "cils.oralWords": "Spisane słowa: {n}",
  "cils.oralOwnWords": "To są twoje słowa spisane z pamięci, a nie transkrypcja nagrania: kurs nie słyszy, co powiedziałeś.",
  "cils.reviewTitle": "Po egzaminie: co powiedziałeś",
  "cils.reviewTopic": "Temat, o którym mówiłeś",
  "cils.reviewLabel": "Bez zegara.",
  "cils.reviewIntro": "Egzamin się skończył. Odsłuchaj siebie i spisz, co powiedziałeś: tylko na tym kurs może cokolwiek sprawdzić.",
  "cils.reviewWrite": "Spisz, co powiedziałeś",
  "cils.reviewHint": "Możesz to pominąć. Wtedy referat powie, że tej części nie było czego sprawdzić.",
  "cils.reviewDone": "Przejdź do referatu",
  "cils.reading": "Czytam…",
  "cils.readingLabel": "Model przeczytał:",
  "cils.playMine": "Odsłuchaj siebie",
  "cils.playFailed": "Nie udało się odtworzyć nagrania.",
  "cils.finish": "Zakończ",
  "cils.resultTitle": "Wynik",
  "cils.notMeasured": "nie liczone tutaj",
  "cils.expiredMark": "(czas minął)",
  "cils.threshold": "Próg: {n} punktów na {max} w każdej sprawności.",
  "cils.verdictBelow": "Przynajmniej jedna sprawność jest poniżej progu, więc egzamin nie byłby zdany. Kapitalizacji tu nie ma: jedna sprawność poniżej progu to cały egzamin od nowa.",
  "cils.verdictUnknown": "Sprawności policzone tutaj są powyżej progu. To nie znaczy „zdane”: pisania i mówienia symulator nie ocenia, a bez nich wynik jest nierozstrzygnięty.",
  "cils.writingNotScored": "Bez oceny: sprawdzamy tylko, czy zadanie zostało wykonane w całości.",
  "cils.again": "Jeszcze raz",
  "cils.backToList": "Inne symulacje",
  "cils.oralSummary": "Bez oceny: sprawdzamy tylko, czy w tym, co spisałeś, jest to, o co prosił temat.",

  /* ---------------- Gramatyka ---------------- */
  "gram.backToIndex": "← Spis treści",
  "gram.kicker": "Materiały",
  "gram.title": "Gramatyka od A do Z",
  "gram.intro": "Pełny sylabus gramatyczny A1 → C2, uporządkowany tematycznie. Zaglądaj tu, kiedy coś w lekcji wymaga szerszego wyjaśnienia.",

  /* ---------------- Koniugator ---------------- */
  "conj.kicker": "Narzędzie",
  "conj.intro": "Wpisz bezokolicznik (także zwrotny, np. svegliarsi). Silnik odmienia formy regularne i ok. 45 najczęstszych nieregularnych.",
  "conj.ph": "np. parlare, essere, capire, svegliarsi",
  "conj.run": "Odmień",
  "conj.notInfinitive": "To nie wygląda na bezokolicznik",
  "conj.notInfinitiveHint": "Bezokolicznik kończy się na -are, -ere, -ire (albo -rsi dla zwrotnych).",
  "conj.irregular": "nieregularny",
  "conj.regular": "regularny",
  "conj.reflexive": "zwrotny",

  /* ---------------- Nazwy czasów ---------------- */
  "tense.pres": "Czas teraźniejszy",
  "tense.passPross": "Czas przeszły złożony",
  "tense.imperf": "Imperfekt",
  "tense.trapPross": "Czas zaprzeszły",
  "tense.futuro": "Czas przyszły prosty",
  "tense.futAnt": "Czas przyszły uprzedni",
  "tense.remoto": "Czas przeszły odległy",
  "tense.condizionale": "Tryb warunkowy teraźniejszy",
  "tense.condPass": "Tryb warunkowy przeszły",
  "tense.cong": "Tryb łączący teraźniejszy",
  "tense.congPass": "Tryb łączący przeszły",
  "tense.congImp": "Tryb łączący imperfekt",
  "tense.congTrap": "Tryb łączący zaprzeszły",
  "tense.imper": "Tryb rozkazujący",

  /* ---------------- Mój słownik ---------------- */
  "lex.kicker": "Słownictwo",
  "lex.intro": "Wszystkie słówka oznaczone gwiazdką. Sortowanie według terminu następnej powtórki.",
  "lex.inDeck": "fiszek w talii",
  "lex.learned": "utrwalonych (3+ powtórki)",
  "lex.dueToday": "czeka na dziś",
  "lex.searchPh": "Szukaj po włosku lub po polsku",
  "lex.today": "dziś",
  "lex.tomorrow": "jutro",
  "lex.inDays": "za {n} dni",
  "lex.delete": "Usuń",
  "lex.empty": "Talia jest pusta",
  "lex.emptyHint": "W każdej lekcji kliknij ⭐ przy słówku, żeby wpadło tutaj i do powtórek.",

  /* ---------------- Postępy ---------------- */
  "prog.kicker": "Statystyki",
  "prog.title": "Twoje postępy",
  "prog.intro": "Wszystko liczone lokalnie, w Twojej przeglądarce. Nic nie wychodzi na zewnątrz.",
  "prog.streak": "dni z rzędu (rekord: {best})",
  "prog.lessonsDone": "ukończonych lekcji",
  "prog.accuracy": "skuteczność odpowiedzi",
  "prog.points": "punktów",
  "prog.lastTwoWeeks": "Ostatnie dwa tygodnie",
  "prog.answers": "{n} odpowiedzi",
  "prog.levels": "Poziomy",
  "prog.ofLessons": "{done} z {total} lekcji",

  /* ---------------- Ustawienia ---------------- */
  "set.kicker": "Konfiguracja",
  "set.intro": "Głos, tempo mowy, kopia postępów.",
  "set.speech": "Mowa",
  "set.voiceSource": "Źródło głosu",
  "set.recorded": "Nagrania lektorskie",
  "set.sentences": "{n} zdań",
  "set.notBuilt": "niewygenerowane",
  "set.systemVoice": "Głos systemowy (Web Speech API)",
  "set.recordedHint": "Zdania kursu czyta lektor neuronowy. Formy z odmiany czasowników, których nie ma w nagraniach, wracają automatycznie do głosu systemowego.",
  "set.notBuiltHint": "Nagrania nie zostały wygenerowane — uruchom <code>scripts/build_audio.py</code>.",
  "set.rate": "Tempo mowy:",
  "set.autoplay": "Automatycznie odtwarzaj nagrania w ćwiczeniach",
  "set.strictAccents": "Wymagaj akcentów (perché, è, città…)",
  "set.sttConsent": "Zgoda na wysyłanie głosu do rozpoznawania",
  "set.sttConsentHint": "Bez niej ćwiczenia z mówieniem niczego nie nagrywają. Co dokładnie się dzieje z nagraniem, opisuje Prywatność.",
  "set.sttConsentOn": "Zgoda udzielona.",
  "set.sttConsentOff": "Zgoda wycofana. Kurs zapyta ponownie przed nagraniem.",
  "talk.checking": "Sprawdzam…",
  "llm.title": "Sprawdzanie odpowiedzi przez model",
  "llm.hint": "Kurs porównuje odpowiedzi litera po literze, więc odrzuca zdania poprawne, ale ułożone inaczej. Możesz dać mu drugą opinię: wklej klucz do dostawcy, którego masz, a kurs zapyta go dopiero wtedy, gdy sam odrzuci twoją odpowiedź. Klucz zostaje na tym urządzeniu, a za zapytania płacisz ty.",
  "llm.moveUp": "Przesuń {name} wyżej w kolejności",
  "llm.orderHint": "Kurs pyta pierwszego dostawcę, który ma klucz, i przechodzi do następnego dopiero wtedy, gdy tamten zawiedzie. Strzałką zmieniasz kolejność.",
  "llm.keySaved": "klucz zapisany — wpisz nowy, aby go zmienić",
  "llm.keyEmpty": "wklej klucz",
  "llm.test": "Sprawdź",
  "llm.testing": "Sprawdzam klucz…",
  "llm.testOk": "Klucz działa.",
  "llm.testFailed": "Klucz nie zadziałał:",
  "llm.noKey": "Najpierw wklej klucz.",
  "llm.saved": "Klucz zapisany.",
  "llm.removed": "Klucz usunięty.",
  "llm.tooShort": "To za krótkie jak na klucz — wklej go w całości.",
  "llm.consent": "Zgoda na wysyłanie odpowiedzi do wybranego dostawcy",
  "llm.consentHint": "Bez niej kurs nie pyta żadnego modelu. Wychodzi twoje zdanie i zdanie wzorcowe, nic więcej. Szczegóły w Prywatności.",
  "llm.consentOn": "Zgoda udzielona.",
  "llm.consentOff": "Zgoda wycofana. Kurs wraca do porównywania litera po literze.",
  "llm.forget": "Usuń wszystkie klucze",
  "llm.forgotten": "Klucze usunięte z tego urządzenia.",
  "llm.keysRejected": "Żaden z twoich kluczy nie został przyjęty. Sprawdź je w Ustawieniach.",
  "set.reviews": "Powtórki",
  "set.retention": "Jak często wracają fiszki",
  "set.retentionRelaxed": "Rzadziej — więcej zapominania",
  "set.retentionDefault": "Domyślnie",
  "set.retentionStrict": "Częściej — mniej zapominania",
  "set.retentionHint": "Kurs wyznacza termin tak, żeby trafić w chwilę, w której słowo dopiero zaczynasz zapominać. Rzadziej to mniej powtórek dziennie i więcej słów, które wypadną; częściej — odwrotnie. Zmiana działa od następnej odpowiedzi, wyznaczone już terminy zostają na miejscu.",
  "set.retentionSaved": "Zapisane. Zadziała przy następnej powtórce.",
  "read.tapHint": "Dotknij dowolnego słowa, żeby zobaczyć znaczenie i dodać je do powtórek.",
  "lookup.cardLabel": "Słowo: {word}",
  "lookup.close": "Zamknij",
  "lookup.meaning": "Znaczenie",
  "lookup.add": "Dodaj do powtórek",
  "lookup.addAnyway": "Dodaj mimo to",
  "lookup.fromForm": "w tekście: {form}",
  "lookup.fromPhrase": "kurs uczy tego w zwrocie: {phrase}",
  "nav.coverage": "Pokrycie",
  "cov.kicker": "Prawdziwy włoski",
  "cov.title": "Ile już masz",
  "cov.intro": "{n} najczęstszych form włoskich. Nie punkty za wierność aplikacji — język, który da się sprawdzić poza nią.",
  "cov.yours": "Twoja talia",
  "cov.yoursWhy": "Formy, które masz w powtórkach, licząc odmianę: „bere” zalicza „bevo”.",
  "cov.course": "Zasięg kursu",
  "cov.courseWhy": "Formy, których kurs w ogóle uczy. To sufit: wyżej nie wejdziesz, choćbyś przerobił wszystko.",
  "cov.ofText": "czyli około {pct}% wszystkiego, co pada w zdaniu",
  "cov.nextUp": "Najbliższe braki",
  "cov.nextUpWhy": "Najczęstsze słowa, których jeszcze nie masz, a kurs je zna. Pod hasłem formy, w jakich stoją w tekście.",
  "cov.add": "Do powtórek",
  "cov.forms": "w tekście: {forms}",
  /* ---------------- Powroty ---------------- */
  "ret.title": "Powroty",
  "ret.intro": "Kurs nie wyśle powiadomienia, bo nie ma serwera, który mógłby je wysłać. Zostawia więc licznik na ikonie i przypomnienie w kalendarzu, który i tak masz otwarty.",
  "ret.badge": "Licznik na ikonie aplikacji",
  "ret.badgeHint": "Pokazuje karty do powtórzenia w ciągu doby. Widzi go każdy, kto spojrzy na twój telefon, dlatego domyślnie jest wyłączony.",
  "ret.badgeAbsent": "Ta przeglądarka nie rysuje licznika na ikonie. Reszta kursu działa bez zmian.",
  "ret.badgeOn": "Licznik włączony",
  "ret.badgeOff": "Licznik wyłączony",
  "ret.reminder": "Godzina przypomnienia",
  "ret.reminderHint": "Pobrany plik dodaje do kalendarza codzienne wydarzenie o tej godzinie. Odwołuje się je w kalendarzu, nie tutaj.",
  "ret.download": "Pobierz przypomnienie (.ics)",
  "ret.saved": "Plik pobrany. Otwórz go, żeby dodać wydarzenie do kalendarza.",
  "ret.install": "Zainstaluj kurs",
  "ret.installManual": "Żeby zainstalować: w menu przeglądarki wybierz „Dodaj do ekranu głównego”. W Safari na iPhonie to przycisk udostępniania.",
  "ret.installed": "Kurs jest już zainstalowany.",
  "ret.installReady": "Ten kurs można zainstalować jak aplikację: otwiera się szybciej, działa bez sieci i ma własną ikonę.",
  "ret.installNow": "Zainstaluj",
  "ret.icsTitle": "Włoski: powtórki",
  "ret.icsBody": "Dziesięć minut wystarczy. Otwórz kurs i zrób sesję dnia.",


  /* ---------------- Swobodna rozmowa ---------------- */
  "nav.chat": "Swobodna rozmowa",
  "chat.kicker": "Rozmowa bez scenariusza",
  "chat.title": "Swobodna rozmowa",
  "chat.intro": "Tu nikt nie napisał wcześniej, co masz powiedzieć. Wybierasz scenę, mówisz swoje, a rozmówca odpowiada po włosku i osobno poprawia to, co warto poprawić.",
  "chat.needsKeyLabel": "Ta część potrzebuje klucza.",
  "chat.needsKeyOn": "Klucz jest ustawiony, a każda tura to jedno zapytanie płatne z twojego konta. Licznik pokazuje, ile tur zostało.",
  "chat.needsKeyOff": "Nie masz jeszcze klucza do modelu ani zgody na rozmowę. Ustawisz oba w Ustawieniach; bez nich reszta kursu działa bez zmian.",
  "chat.none": "Brak scen do rozmowy.",
  "chat.notFound": "Nie ma takiej sceny.",
  "chat.backToList": "← Sceny",
  "chat.placeholder": "Napisz po włosku…",
  "chat.send": "Wyślij",
  "chat.waiting": "Rozmówca pisze…",
  "chat.failed": "Odpowiedź nie przyszła. Tura nie została policzona — spróbuj jeszcze raz.",
  "chat.budgetSpent": "Limit swobodnej rozmowy na tę sesję wyczerpany. Sędzia ćwiczeń działa dalej; rozmowa wróci po odświeżeniu strony.",
  "chat.left": "Zostało tur: {n}",
  "chat.over": "Koniec rozmowy. Tur: {n}.",
  "chat.noFixes": "Rozmówca nie poprawił niczego.",
  "chat.again": "Jeszcze raz",
  "chat.others": "Inne sceny",
  "chat.consent": "Zgoda na swobodną rozmowę",
  "chat.consentHint": "Osobna od tej wyżej, bo wychodzi co innego: nie jedno zdanie, a cała rozmowa z danej sceny. Wycofasz ją tutaj, kiedy zechcesz.",
  "chat.consentOn": "Zgoda na rozmowę włączona",
  "chat.consentOff": "Zgoda na rozmowę wycofana",
  "chat.sc.bar": "W barze",
  "chat.sc.vicino": "Sąsiad na klatce",
  "chat.sc.mercato": "Na targu",
  "chat.sc.ufficio": "W urzędzie",
  "chat.sc.colloquio": "Rozmowa o pracę",
  "chat.sc.medico": "U lekarza",

  "anki.title": "Talia dla Anki",
  "anki.hint": "Słowa i tłumaczenia w formacie, który czyta Anki, arkusz i każdy inny program do fiszek.",
  "anki.noSchedule": "Terminy powtórek NIE wyjeżdżają: wychodzą słowa, nie kalendarz. W Anki fiszki zaczną od zera.",
  "anki.export": "⬇ Eksportuj talię",
  "anki.import": "⬆ Wczytaj talię",
  "anki.exported": "Zapisano {n} fiszek.",
  "anki.nothingToExport": "Talia jest pusta.",
  "anki.emptyFile": "W pliku nie ma ani jednej fiszki.",
  "anki.summary": "Dodam {add}, zaktualizuję {upd}, pominę {skip}.",
  "anki.confirm": "Wczytaj",
  "anki.cancel": "Anuluj",
  "anki.cancelled": "Nic nie zostało zmienione.",
  "anki.imported": "Wczytano {n} fiszek.",
  "anki.errRagged": "Wiersze mają różną liczbę kolumn — plik jest uszkodzony albo separator siedzi w treści. Nic nie wczytano.",
  "anki.errTooManyRows": "Plik ma za dużo wierszy jak na talię. Nic nie wczytano.",
  "anki.errTooBig": "Plik jest za duży. Nic nie wczytano.",
  "anki.errUnterminated": "W pliku jest niedomknięty cudzysłów, więc dalszej treści nie da się odczytać. Nic nie wczytano.",
  "nav.falsi": "Fałszywi przyjaciele",
  "fa.kicker": "Twój język ci nie pomaga",
  "fa.title": "Fałszywi przyjaciele",
  "fa.intro": "{n} słów, które po włosku znaczą co innego, niż podpowiada polski. Lista jest inna dla każdego języka, bo pułapka zależy od tego, co masz w głowie.",
  "fa.looksLike": "wygląda jak",
  "fa.actually": "znaczy",
  "fa.startQuiz": "Sprawdź się",
  "fa.backToList": "← Lista",
  "fa.quizTitle": "Które znaczenie jest prawdziwe",
  "fa.quizIntro": "Dziesięć słów. Zła odpowiedź to dokładnie ta pułapka, o którą chodzi.",
  "fa.qWhat": "Co naprawdę znaczy „{word}”?",
  "fa.progress": "{i} z {n}",
  "fa.next": "Dalej →",
  "fa.done": "{hit} z {n} dobrze.",
  "fa.again": "Wróć do listy",
  "fa.none": "Dla tego języka nie ma jeszcze wpisów.",
  "fa.noneHint": "Pułapki pisze się osobno pod każdy język, więc lista rośnie językami, nie tłumaczeniami.",
  "nav.shadow": "Powtarzanie",
  "sh.kicker": "Twój głos obok lektora",
  "sh.title": "Powtarzanie",
  "sh.intro": "Posłuchaj zdania, powtórz je i odsłuchaj oba nagrania jedno po drugim. Nikt tego nie ocenia: różnicę słyszysz sam.",
  "sh.playNative": "▶ Lektor",
  "sh.record": "🎤 Nagraj",
  "sh.stopRecording": "■ Zatrzymaj",
  "sh.recordAgain": "🎤 Jeszcze raz",
  "sh.playMine": "▶ Moje nagranie",
  "sh.playBoth": "▶ Oba po kolei",
  "sh.next": "Inne zdanie",
  "sh.stateNative": "Gra lektor…",
  "sh.stateMine": "Twoje nagranie…",
  "sh.stateRecording": "Nagrywam. Naciśnij, żeby zatrzymać.",
  "sh.stateDone": "Nagrane, {s} s.",
  "sh.notSaved": "Twoje nagrania nie są nigdzie zapisywane: znikają, gdy zamkniesz stronę.",
  "sh.noMicLabel": "Bez mikrofonu.",
  "sh.listenOnly": "Zostaje słuchanie lektora.",
  "sh.noSentences": "Nie ma zdań z nagraniem do powtarzania.",
  "rec.errInsecure": "Ta strona nie jest w bezpiecznym kontekście, więc przeglądarka nie da dostępu do mikrofonu.",
  "rec.errNoMic": "Ta przeglądarka nie udostępnia mikrofonu stronom.",
  "rec.errNoRecorder": "Ta przeglądarka nie umie nagrywać dźwięku.",
  "rec.errNoFormat": "Ta przeglądarka nie ma formatu, w którym umielibyśmy nagrać.",
  "rec.errDenied": "Nie ma zgody na mikrofon. Możesz jej udzielić w pasku adresu.",
  "rec.errNoDevice": "Nie znaleziono mikrofonu.",
  "rec.errBusy": "Nagrywanie już trwa.",
  "rec.errFailed": "Nagrywanie się nie udało.",
  "stt.consentTitle": "Twój głos wyjdzie poza przeglądarkę",
  "stt.consentBody": "Rozpoznawanie mowy nie liczy się na tym urządzeniu: przeglądarka wysyła nagranie Twojego głosu na serwer swojego dostawcy i odsyła tekst. Nie mamy nad tym kontroli i bez własnego serwera nie da się inaczej. Dotyczy tylko ćwiczeń z mikrofonem; reszta kursu zostaje u Ciebie.",
  "stt.consentYes": "Zgadzam się",
  "stt.consentNo": "Nie, dziękuję",
  "stt.consentLater": "Bez zgody te ćwiczenia zamieniają się w pisane.",
  "nav.speed": "Tempo",
  "sp.kicker": "Kiedy nikt nie zwalnia",
  "sp.title": "Tempo mowy",
  "sp.intro": "Zdanie leci najpierw w normalnym tempie. Spowolnienie jest pod ręką, ale to koło ratunkowe, nie punkt wyjścia: wolnego włoskiego nie mówi nikt poza kursami.",
  "sp.normal": "▶ Normalnie",
  "sp.slow": "▶ Wolniej",
  "sp.fast": "▶ Szybciej",
  "sp.reveal": "Pokaż zdanie",
  "sp.next": "Następne",
  "sp.progress": "Zdanie {i} z {n}",
  "sp.statePlaying": "Gra, tempo {r}×",
  "sp.fullSpeed": "Zrozumiane w normalnym tempie.",
  "sp.usedSlow": "Tym razem z pomocą spowolnienia. To jest w porządku i to jest właśnie to, co ćwiczysz.",
  "sp.noSentences": "Nie ma nagranych zdań do słuchania.",
  "cov.nothingLeft": "Z tej listy nie zostało nic, czego kurs uczy, a Ty nie masz.",
  "cov.noList": "Lista częstości nie została wczytana.",
  "cov.source": "Źródło: {source}, {sentences} zdań, licencja {license}.",
  "lookup.unknown": "Tego słowa nie ma w kursie.",
  "lookup.grammarWord": "To wyraz gramatyczny — rodzajnik, przyimek albo zaimek.",
  "lookup.maybe": "Forma podstawowa to może: {forms}",
  "lookup.writeIt": "wpisz znaczenie",
  "lookup.needMeaning": "Najpierw wpisz znaczenie.",
  "lookup.added": "„{word}” trafiło do powtórek.",
  "set.testVoice": "🔊 Test głosu",
  "set.backup": "Kopia postępów",
  "set.backupHint": "Postępy siedzą w pamięci tej przeglądarki. Wyeksportuj plik, jeśli zmieniasz komputer albo czyścisz dane.",
  "set.export": "⬇ Eksportuj",
  "set.import": "⬆ Importuj",
  "set.reset": "Wyczyść wszystko",
  "set.support": "Wsparcie przeglądarki",
  "set.voicesUsed": "Isabella (główny) i Giuseppe (rozmówca)",
  "set.tts": "Synteza systemowa (TTS)",
  "set.ttsUse": "tryb awaryjny i odmiana czasowników",
  "set.stt": "Rozpoznawanie mowy (STT)",
  "set.sttUse": "ćwiczenia mówione",
  "set.offline": "Praca bez sieci",
  "set.offlineUse": "Kurs zapisany w przeglądarce: po pierwszym wczytaniu działa bez internetu, a nagrania dochodzą do pamięci w miarę słuchania.",
  "set.offlineFile": "Plik otwarty z dysku (file://). Przeglądarki nie pozwalają tu na tryb offline — kurs i tak działa, ale nic się nie zapisuje na później.",
  "set.offlineWaiting": "za chwilę",
  "set.offlineNeedsServer": "potrzebny serwer",
  "set.storage": "Trwała pamięć",
  "set.storageUse": "chroni postępy przed sprzątaniem pamięci przez przeglądarkę",
  "set.storageGranted": "przyznana",
  "set.storageDenied": "nieprzyznana",
  "set.storageUnknown": "niedostępna",
  "set.italianVoices": "Głosy włoskie w systemie",
  "set.fallbackOnly": "używane tylko w trybie awaryjnym",
  "set.works": "działa",
  "set.absent": "brak",
  "set.absentTyping": "brak — tryb pisany",
  "set.imported": "Postępy wczytane.",
  "set.errNotSave": "Ten plik nie zawiera zapisu postępów.",
  "set.errNoVersion": "Plik nie ma poprawnego numeru wersji: prawdopodobnie nie pochodzi z tego kursu.",
  "set.errFromFuture": "Plik pochodzi z nowszej wersji kursu. Zaktualizuj stronę i spróbuj ponownie.",
  "set.errBadField": "Pole {field} ma w pliku zły typ. Plik jest uszkodzony.",
  "set.errTooBig": "Plik jest za duży, żeby był zapisem postępów.",
  "set.importFailed": "Nie udało się wczytać pliku.",
  "set.resetConfirm": "Na pewno? Wszystkie postępy, fiszki i statystyki zostaną skasowane.",
  "set.resetDone": "Wyczyszczone.",
  "core.saveBlocked": "Pamięć przeglądarki jest pełna: postępy nie są już zapisywane. Wyeksportuj je w Ustawieniach i zwolnij miejsce.",
  "core.storagePruned": "Zabrakło miejsca: najstarsze karty powtórek zostały usunięte, żeby zachować postępy lekcji.",
  "core.noticeDismiss": "Zamknij komunikat",
  "core.backupDue": {
    one: "Od ostatniej kopii minęła {n} ukończona lekcja. Kurs trzyma postępy tylko w tej przeglądarce: zapisz kopię, bo wyczyszczonej pamięci nie ma z czego odtworzyć.",
    few: "Od ostatniej kopii minęły {n} ukończone lekcje. Kurs trzyma postępy tylko w tej przeglądarce: zapisz kopię, bo wyczyszczonej pamięci nie ma z czego odtworzyć.",
    many: "Od ostatniej kopii minęło {n} ukończonych lekcji. Kurs trzyma postępy tylko w tej przeglądarce: zapisz kopię, bo wyczyszczonej pamięci nie ma z czego odtworzyć.",
    other: "Od ostatniej kopii minęło {n} ukończonych lekcji. Kurs trzyma postępy tylko w tej przeglądarce: zapisz kopię, bo wyczyszczonej pamięci nie ma z czego odtworzyć."
  },
  "core.backupSave": "Zapisz kopię",
  "pwa.updateReady": "Jest nowa wersja kursu. Aktualizacja trwa chwilę i nie rusza postępów ani fiszek.",
  "pwa.updateNow": "Zaktualizuj",

  /* ---------------- Znaczenia czasowników w podpowiedziach koniugatora ---------------- */
  "verb.essere": "być",
  "verb.avere": "mieć",
  "verb.fare": "robić",
  "verb.dire": "mówić / powiedzieć",
  "verb.andare": "iść / jechać",
  "verb.venire": "przychodzić",
  "verb.stare": "być / czuć się",
  "verb.dare": "dawać",
  "verb.sapere": "wiedzieć / umieć",
  "verb.potere": "móc",
  "verb.volere": "chcieć",
  "verb.dovere": "musieć",
  "verb.vedere": "widzieć",
  "verb.parlare": "mówić / rozmawiać",
  "verb.mangiare": "jeść",
  "verb.bere": "pić",
  "verb.prendere": "brać",
  "verb.mettere": "kłaść",
  "verb.leggere": "czytać",
  "verb.scrivere": "pisać",
  "verb.capire": "rozumieć",
  "verb.finire": "kończyć",
  "verb.preferire": "woleć",
  "verb.dormire": "spać",
  "verb.partire": "wyjeżdżać",
  "verb.aprire": "otwierać",
  "verb.chiudere": "zamykać",
  "verb.comprare": "kupować",
  "verb.pagare": "płacić",
  "verb.cercare": "szukać",
  "verb.lavorare": "pracować",
  "verb.studiare": "uczyć się",
  "verb.giocare": "grać",
  "verb.viaggiare": "podróżować",
  "verb.conoscere": "znać",
  "verb.credere": "wierzyć",
  "verb.vivere": "żyć",
  "verb.nascere": "urodzić się",
  "verb.morire": "umierać",
  "verb.piacere": "podobać się",
  "verb.rimanere": "zostawać",
  "verb.uscire": "wychodzić",
  "verb.salire": "wchodzić / wsiadać",
  "verb.scendere": "schodzić / wysiadać",
  "verb.tenere": "trzymać",
  "verb.scegliere": "wybierać",
  "verb.perdere": "tracić / gubić",
  "verb.vincere": "wygrywać",
  "verb.correre": "biegać",
  "verb.chiedere": "pytać / prosić",
  "verb.rispondere": "odpowiadać",
  "verb.decidere": "decydować",
  "verb.spegnere": "gasić / wyłączać",
  "verb.tradurre": "tłumaczyć",
  "verb.svegliarsi": "budzić się",
  "verb.alzarsi": "wstawać",
  "verb.lavarsi": "myć się",
  "verb.vestirsi": "ubierać się",
  "verb.chiamarsi": "nazywać się",
  "verb.divertirsi": "bawić się",
  "verb.annoiarsi": "nudzić się",
  "verb.arrabbiarsi": "złościć się",
  "verb.innamorarsi": "zakochać się",
  "verb.ricordarsi": "pamiętać",
  "verb.dimenticare": "zapominać",
  "verb.aiutare": "pomagać",
  "verb.arrivare": "przyjeżdżać",
  "verb.tornare": "wracać",
  "verb.entrare": "wchodzić",
  "verb.diventare": "stawać się",
  "verb.riuscire": "udawać się",
  "verb.succedere": "zdarzać się",

  /* ── Prywatność ─────────────────────────────────────────────
     Tekst źródłowy: docs/PRIVACY.it.md (po włosku, bo w tym języku
     administrator podpisuje to, co dokument twierdzi). Każde zdanie ma
     swój wiersz w specs/004-lancio-pubblico/riscontri.md, z poleceniem,
     które sprawdziło je na kodzie.

     Adresu ani nazwiska tu nie ma: wchodzą jako {mail} i {name}
     z assets/js/views-privacy.js, więc stoją w jednym miejscu, a nie
     w pięciu słownikach. Nazwy przycisków ({export}, {import}, {reset})
     przychodzą z kluczy set.*, czyli z tych samych napisów, które uczeń
     widzi w Ustawieniach. */
  "privacy.kicker": "Twoje dane",
  "privacy.title": "Prywatność",
  "privacy.intro": "Co zostaje na twoim urządzeniu, co z niego wychodzi i dlaczego.",
  "privacy.updated": "Ostatnia aktualizacja: {date}.",
  "privacy.toSettings": "Otwórz Ustawienia",

  "privacy.chi.h": "Kto przetwarza dane",
  "privacy.chi.p1": "Administratorem danych jest {name}. W każdej sprawie dotyczącej tej informacji albo samych danych: {mail}.",

  "privacy.breve.h": "W skrócie",
  "privacy.breve.p1": "Impara l'Italiano nie ma kont ani serwera. Nie pyta cię o imię. Twoje postępy zostają w przeglądarce, której właśnie używasz, i nikt poza tobą ich nie odczyta: nie używamy ciasteczek, nie mamy zainstalowanego żadnego systemu statystyk, strona nie łączy się z żadną obcą witryną.",
  "privacy.breve.p2": "Są trzy rzeczy, które mimo to wychodzą z twojego urządzenia, i ta informacja istnieje przede wszystkim po to, żeby ci je powiedzieć: twój głos, kiedy używasz ćwiczeń z wymową; twoje pisane odpowiedzi, ale tylko jeśli sam włączysz sprawdzanie przez model; oraz twój adres IP, który widzi ten, kto hostuje stronę.",

  "privacy.dispositivo.h": "Co zostaje na twoim urządzeniu",
  "privacy.dispositivo.p1": "Kurs zapisuje w pamięci lokalnej przeglądarki (localStorage, pod kluczem linguai.italiano.v2) wszystko, co pozwala wrócić do miejsca, w którym nauka się zatrzymała:",
  "privacy.dispositivo.l1": "ukończone lekcje, punkty i passę kolejnych dni;",
  "privacy.dispositivo.l2": "fiszki powtórek razem z ich terminami;",
  "privacy.dispositivo.l3": "zeszyt błędów, czyli które ćwiczenia poszły źle i z jakiego zagadnienia;",
  "privacy.dispositivo.l4": "teksty napisane w ćwiczeniach pisemnych;",
  "privacy.dispositivo.l5": "ustawienia, w tym język wyjaśnień i odpowiedź w sprawie rozpoznawania mowy.",
  "privacy.dispositivo.p2": "Te dane nigdzie nie są wysyłane. Nie ma miejsca, w którym moglibyśmy je przeczytać, bo nie ma serwera kursu.",
  "privacy.dispositivo.p3": "Możesz je zabrać albo skasować, kiedy zechcesz, w Ustawieniach: „{export}” pobiera plik z całym profilem, „{import}” wgrywa go na innym urządzeniu, „{reset}” kasuje wszystko. Skasowanie danych przeglądania działa tak samo jak „{reset}” i jest nieodwracalne: jeśli zależy ci na postępach, eksportuj je co jakiś czas.",

  "privacy.voce.h": "Twój głos w ćwiczeniach wymowy",
  "privacy.voce.p1": "To jest ta część, która się liczy, i nie chcemy, żeby cię zaskoczyła.",
  "privacy.voce.p2": "W ćwiczeniach, w których mówisz na głos, a kurs sprawdza wypowiedziane zdanie, używamy rozpoznawania mowy wbudowanego w przeglądarkę. W przeglądarkach, które je udostępniają (Chrome, Edge, Safari), nie jest to funkcja działająca na twoim urządzeniu: nagranie twojego głosu jedzie na serwery producenta przeglądarki, tam zamienia się w tekst i wraca do kursu jako tekst. Nie przechodzi przez nas, nie dostajemy go i nie przechowujemy, ale i tak wychodzi z twojego urządzenia, a o tym trzeba wiedzieć wcześniej, nie później.",
  "privacy.voce.p3": "Dlatego kurs pyta o zgodę przed pierwszym razem, a bez zgody ćwiczenie nie nagrywa niczego. Zgoda obejmuje wszystkie ćwiczenia tego rodzaju i zostaje zapisana w ustawieniach.",
  "privacy.voce.p4": "Możesz ją wycofać, kiedy zechcesz, w Ustawieniach, przy rozpoznawaniu mowy: od tego momentu kurs znowu pyta, zanim cokolwiek nagra.",
  "privacy.voce.p5": "Co producent przeglądarki robi z nagraniem po jego otrzymaniu, nie zależy od nas i nie umiemy tego sprawdzić; odpowiedź jest w informacji o prywatności przeglądarki, której używasz. Inaczej wygląda ćwiczenie z powtarzaniem, w którym słuchasz samego siebie: tam nagranie zostaje w stronie, nigdzie się nie zapisuje i znika przy zamknięciu karty albo przy przejściu do następnego zdania.",
  "privacy.dispositivo.p4": "Jeśli włączyłeś sprawdzanie odpowiedzi przez model, wklejone klucze też leżą w pamięci przeglądarki, ale w osobnym pojemniku (`linguai.llm.v1`), poza profilem. Powód jest praktyczny: {export} pobiera profil, a plik, który zachęcamy cię trzymać i przenosić między urządzeniami, nie powinien zawierać danych, za które płacisz. Dlatego {reset} kasuje również klucze.",
  "privacy.modello.h": "Twoje odpowiedzi, jeśli włączysz sprawdzanie przez model",
  "privacy.modello.p1": "Ta część kursu jest wyłączona, dopóki sam jej nie włączysz, i pozostaje wyłączona dla każdego, kto nic nie zrobi.",
  "privacy.modello.p2": "Kurs porównuje twoje odpowiedzi ze wzorcowymi litera po literze, a ta metoda myli się w jedną stronę: odrzuca zdania poprawne, ale ułożone inaczej niż wzorzec. Możesz dać jej drugą opinię. W {settings} jest miejsce, gdzie wklejasz klucz do dostawcy modeli, którego już używasz: Google Gemini, Groq, OpenAI albo Anthropic. Od tej chwili — i dopiero wtedy, gdy porównanie na urządzeniu już odrzuciło twoją odpowiedź — kurs wysyła wybranemu dostawcy dwa zdania, twoje i wzorcowe, i dostaje ocenę. Trzy rzeczy, o których warto wiedzieć wcześniej:",
  "privacy.modello.p3": "Jest drugie miejsce, w którym to się dzieje, i wychodzi tam więcej. W ćwiczeniach z pisania możesz poprosić model o opinię o swoim tekście, a wtedy wychodzi cały tekst razem z poleceniem ćwiczenia. Nigdy nie dzieje się to samo z siebie: jest przycisk, naciskasz go ty, a dopóki go nie naciśniesz, twoje wypracowanie zostaje na miejscu. Ta opinia nie jest oceną i nie rusza ani twoich postępów, ani fiszek.",
  "privacy.modello.l1": "klucz zostaje na tym urządzeniu. Nie przechodzi przez nas w żadnym momencie: idzie z twojej przeglądarki do dostawcy i tyle. Nie trafia nawet do pliku, który pobierasz przez {export};",
  "privacy.modello.l2": "za zapytania płacisz ty, na umowie, którą masz z tym dostawcą. My nie mamy jak ich zobaczyć ani policzyć;",
  "privacy.modello.l3": "co dostawca robi z twoim zdaniem po jego otrzymaniu, nie zależy od nas i nie możemy tego sprawdzić — dokładnie tak samo jak przy rozpoznawaniu mowy. Odpowiedź jest w polityce prywatności dostawcy, którego wybrałeś.",
  "privacy.modello.p4": "Także tutaj kurs pyta cię o zgodę, i jest to zgoda osobna od tej na rozpoznawanie mowy: przyjęcie jednej nie jest przyjęciem drugiej, a wycofanie jednej nie wycofuje drugiej. Wycofasz ją, kiedy zechcesz, w {settings}, gdzie możesz też skasować wszystkie klucze naraz. Bez klucza albo bez zgody kurs nie kontaktuje się z żadnym dostawcą i zachowuje się dokładnie tak jak wcześniej.",
  "privacy.modello.p5": "Jest i trzecie miejsce, symulacja egzaminu, i wychodzi tam jeszcze więcej — bez naciskania czegokolwiek. Na końcu podejścia, jeśli sprawdzanie przez model jest już włączone, kurs wysyła dostawcy dwie rzeczy: tekst z prowy pisemnej razem z poleceniem oraz to, co spisałeś po prowie ustnej. Warto wiedzieć, czym te teksty są: polecenia egzaminacyjne pytają o twoją dzielnicę, twoją pracę i o to, jak trafiłeś do Włoch, więc prawdopodobnie napisałeś tam, gdzie mieszkasz i co robisz. Jeśli wolisz, żeby nie wychodziły, wyłącz sprawdzanie przez model w {settings} przed rozpoczęciem: symulacja działa tak samo, punkty ze słuchania i czytania są te same, a obie produkcje zostają przy wykrywaniu, które kurs robi sam, na twoim urządzeniu. Twój głos tą drogą nie wychodzi nigdy: nagranie zostaje w pamięci strony i znika, kiedy ją zamkniesz.",
  "privacy.modello.p6": "I jest czwarte miejsce, w którym wychodzi najwięcej: swobodna rozmowa. Tam nie wyrusza jedno zdanie, tylko wszystko, co powiedziałeś w tej scenie, tura po turze, i rusza od nowa przy każdej następnej turze, bo model potrzebuje ciągu dalszego, żeby odpowiedzieć. Dlatego ma własną zgodę, osobną od tej na sprawdzanie odpowiedzi: kto zgodził się na sprawdzenie jednego zdania, nie zgodził się tym samym na wysłanie całej rozmowy, a wyłączenie jednej nie wyłącza drugiej. Rozmowa nie jest zapisywana: żyje w pamięci strony, znika przy zamknięciu albo przeładowaniu i nie trafia do pliku z {export}. Każda tura to płatne zapytanie na twojej umowie, i właśnie dlatego licznik pozostałych tur stoi na ekranie od pierwszej chwili.",

  "privacy.cache.h": "Kopie, które kurs trzyma, żeby działać bez sieci",
  "privacy.cache.p1": "Kurs działa też bez internetu. Żeby to było możliwe, przeglądarka trzyma kopię plików kursu: kodu, tekstów lekcji i nagrań lektorów. To są nasze pliki, takie same dla wszystkich, i nie ma w nich niczego, co dotyczy ciebie. Znikają, kiedy skasujesz dane strony w przeglądarce.",

  "privacy.hosting.h": "Kto udostępnia stronę",
  "privacy.hosting.p1": "Kurs jest opublikowany na GitHub Pages. Jak przy każdej stronie, ten, kto ją udostępnia, zapisuje przychodzące żądania, a w tych żądaniach jest twój adres IP. To jedyne przetwarzanie, o którym nie decydujemy my, i nie da się go uniknąć: bez kogoś, kto poda stronę, nie ma strony. Co się z tym dzieje dalej, opisują warunki GitHuba.",

  "privacy.base.h": "Na jakiej podstawie",
  "privacy.base.p1": "Dane zapisane w przeglądarce służą do działania kursu, który otworzyłeś, i produkujesz je ty, używając go: na tym opiera się przetwarzanie. Rozpoznawanie mowy i sprawdzanie odpowiedzi przez model opierają się natomiast na twojej zgodzie, która jest dobrowolna, o którą pytamy wcześniej i którą możesz wycofać. To dwie osobne zgody i traktujesz je osobno.",
  "privacy.base.p2": "Nie profilujemy nikogo i nie podejmujemy automatycznych decyzji na twój temat. Nie przekazujemy niczego nikomu: nie ma czego przekazywać.",

  "privacy.diritti.h": "Twoje prawa",
  "privacy.diritti.p1": "Rozporządzenie europejskie daje ci prawo dostępu do danych, ich sprostowania, usunięcia i przeniesienia. Tutaj korzystasz z nich bez pytania o pozwolenie, bo dane są już w twoich rękach: „{export}” to przeniesienie, „{reset}” to usunięcie, a ekran postępów to dostęp.",
  "privacy.diritti.p2": "Jeśli masz pytanie, na które ta strona nie odpowiada, napisz na {mail}. Jeśli uważasz, że coś tu się nie zgadza, możesz zwrócić się do organu nadzorczego w swoim kraju.",

  "privacy.zmiany.h": "Jeśli ta informacja się zmieni",
  "privacy.zmiany.p1": "Data na górze mówi, kiedy tekst był pisany ostatni raz. Tekst leży w repozytorium projektu razem z kodem, który opisuje, więc każda zmiana jest jawna i ma datę w historii zmian.",

  /* ---------------- Pięć minut: sesja fiszek z końcem ---------------- */
  "nav.flash": "Pięć minut",
  "flash.kicker": "PIĘĆ MINUT",
  "flash.title": "Krótka sesja, która się kończy",
  "flash.intro": "Pięć minut albo dwadzieścia fiszek, co nastąpi pierwsze. Każda odpowiedź zapisuje się od razu, więc zamknięcie karty w połowie niczego nie traci.",
  "flash.ready": { one: "Ta sesja ma {n} fiszkę.", few: "Ta sesja ma {n} fiszki.", many: "Ta sesja ma {n} fiszek.", other: "Ta sesja ma {n} fiszki." },
  "flash.start": "Zaczynam",
  "flash.timeLeft": "Pozostały czas",
  "flash.oneMinute": "Została minuta.",
  "flash.timeUp": "Czas minął. Dokończ tę fiszkę, a sesja się zamknie.",
  "flash.end.cap": "Dwadzieścia fiszek. To była cała sesja.",
  "flash.end.time": "Pięć minut minęło.",
  "flash.end.empty": "Na razie nie ma więcej fiszek.",
  "flash.again": "Jeszcze pięć minut",
  "flash.emptyTitle": "Teraz nie ma nic do powtórzenia",
  "flash.emptyText": "Wszystkie słowa z tego miejsca są już w twojej talii i żadne nie czeka na powtórkę. Wybierz inną jednostkę albo wróć później.",
  "flash.fromNew": { one: "W tym {n} nowe słowo z twoich lekcji.", few: "W tym {n} nowe słowa z twoich lekcji.", many: "W tym {n} nowych słów z twoich lekcji.", other: "W tym {n} nowego słowa z twoich lekcji." },
  "flash.unitLabel": "Skąd brać nowe słowa",
  "flash.unitDone": "Z lekcji, które już zaliczyłeś",
  "flash.unitFirst": "Z pierwszej jednostki kursu",
  "flash.emptyQuota": "Dziesięć nowych słów na dziś już za tobą. Jutro przyjdą kolejne, a te dzisiejsze wrócą, kiedy przyjdzie ich pora.",
  "flash.loading": "Wczytuję słowa z kursu…",
  "flash.newWord": "nowe słowo",
  "flash.meaning": "Co to znaczy?",
  "flash.show": "Pokaż",
  "flash.tryIt": "Pięć minut z nowymi słowami",
  "flash.replay": "Posłuchaj jeszcze raz",
  "flash.listenPick": "Posłuchaj i wybierz, co usłyszałeś.",
  "flash.listenWrite": "Posłuchaj i zapisz po włosku.",
  "guide.s.cinque": "Sesja fiszek, która kończy się po pięciu minutach albo dwudziestu fiszkach."

});
